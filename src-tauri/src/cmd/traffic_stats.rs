...
pub async fn record_traffic_usage(
    ...
    // 更新统计
    update_subscription_stats(&mut storage, &subscription_uid, &subscription_name)
        .await
    ...
    // 检查并生成警告
    check_and_generate_alerts(&mut storage, &subscription_uid)
        .await
...
/// 获取订阅流量统计
#[tauri::command]
pub async fn get_subscription_traffic_stats(
    subscription_uid: String,
) -> CmdResult<SubscriptionTrafficStats> {
    logging!(
        info,
        Type::Cmd,
        true,
...
pub async fn get_traffic_overview() -> CmdResult<TrafficOverview> {
    ...
    let active_subscriptions = storage
        .stats
        .values()
        .filter(|s| {
            s.last_used.is_some()
    ...
    let total_sessions: u64 = storage.stats.values().map(|s| s.session_count).sum();

    let total_duration: u64 = storage
        .stats
        .values()
    ...
    let this_month_usage: u64 = storage
        .stats
        .values()
    ...
    let critical_alerts_count = storage
        .alerts
        .iter()
        .filter(|a| {
            matches!(
...
pub async fn get_traffic_alerts(include_read: Option<bool>) -> CmdResult<Vec<TrafficAlert>> {
    ...
    let alerts: Vec<TrafficAlert> = storage
        .alerts
        .iter()
...
pub async fn cleanup_traffic_history(days_to_keep: u32) -> CmdResult<u64> {
    ...
    // 重新计算统计数据
    let uids_to_update: Vec<String> = storage
        .records
        .iter()
    ...
    for uid in uids_to_update {
        let subscription_name = get_subscription_name(&uid)
            .await
            .unwrap_or_else(|| "Unknown".to_string());
        update_subscription_stats(&mut storage, &uid, &subscription_name)
            .await
            .map_err(|e| format!("Failed to update subscription stats: {}", e))?;
    }
