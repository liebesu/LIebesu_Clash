...
/// 计算任务统计信息
fn calculate_task_statistics(task_id: &str, history: &[TaskExecutionResult]) -> TaskStatistics {
    ...
    let avg_duration_ms = if total_executions > 0 {
        history.iter().filter_map(|r| r.duration_ms).sum::<u64>() as f64 / total_executions as f64
    } else {
        0.0
    };
